...
(function() {
  var CLIENT_ID, Event, MAP_STATUS, Place, SCOPES, TIME_ZONE_HOST, authorizeFunction, events, geocoder, getLocalizedString, handleAuthResult, handleClientLoad, initializeDOM, initializeGoogleMaps, localTime, localize, map, saveMapStatus, setLocalExpressionInto, timeZone,
    _this = this;
  ...
  localTime = function(date, offset) {
    var offsetHours, offsetMinutes, timeDifference, twoDigitsFormat;
    ...
    timeDifference = (n >= 0 ? '+' : '-') + twoDigitsFormat(offsetHours) + twoDigitsFormat(offsetMinutes);
    return new Date(date.getTime() - offset * 1000).toISOString().replace(/Z/, timeDifference);
  };
