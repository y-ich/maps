...
(function() {
  var APP_NAME, CLIENT_ID, Event, MAP_STATUS, Place, SCOPES, TIME_ZONE_HOST, calendars, clearDirections, compareEventResources, currentCalendar, currentPlace, directions, directionsCondition, directionsRenderer, getLocalizedString, getTimeZone, handleAuthResult, initializeDOM, initializeGoogleMaps, localTime, localize, map, mapSum, saveMapStatus, searchDirections, setLocalExpressionInto, showDirections, spinner, sum, timeDifference,
  ...
  showDirections = function(directionsResult, routeIndex) {
    if (routeIndex == null) {
      routeIndex = 0;
    }
    map.setMapTypeId((function() {
      switch (directionsResult.travelMode) {
  ...
  Event = (function() {
    ...
    Event.geocoder = new google.maps.Geocoder();
    ...
    Event.prototype.geocode = function(callback) {
      ...
      Event.geocoder.geocode(options, function(results, status) {
        switch (status) {
  ...
  initializeDOM = function() {
    ...
    $('#button-prev, #button-next').on('click', function() {
      ...
      if (directions != null) {
        ...
        return showDirections(directions.results[directions.index], directions.routeIdex);
      } else {
