...
(function() {
  var APP_NAME, CLIENT_ID, DirectionsController, Event, LOCAL_CALENDAR, MAP_STATUS, Place, SCOPES, TIME_ZONE_HOST, calendars, compareEventResources, currentPlace, directionsCondition, directionsController, findCalendarById, getLocalizedString, getTimeZone, handleAuthResult, initializeDOM, initializeGoogleMaps, localCalendar, localTime, localize, map, mapSum, saveMapStatus, searchDirections, setLocalExpressionInto, spinner, sum, timeDifference,
  ...
  findCalendarById = function(calendars, id) {
  ...
  handleAuthResult = function(result) {
    if ((result != null) && (result.error == null)) {
      ...
      return $('#button-calendar').attr('disabled', null);
    }
  };
  ...
  searchDirections = function(origin, destination, departureTime, callback) {
    ...
    for (_i = 0, _len = TRAVEL_MODES.length; _i < _len; _i++) {
      ...
      new google.maps.DirectionsService().route(options, (function(travelMode, deferred) {
  ...
  Event = (function() {
    ...
    Event.changeCalendarId = function(id) {
      var e, _i, _len, _ref, _results;
      _ref = Event.events;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        e = _ref[_i];
        _results.push(e.calendarId = id);
      }
      return _results;
    };

    Event.getDate = function(resource, startOrEnd) {
      var _ref;
      return new Date((_ref = resource[startOrEnd].dateTime) != null ? _ref : resource[startOrEnd].date + (startOrEnd === 'start' ? 'T00:00:00Z' : 'T23:59:59Z'));
    };
    ...
    Event.prototype.update = function() {
      var e, events, i, _i, _len;
      if (this.calendarId != null) {
        if (this.calendarId === 'local') {
          if (localStorage[LOCAL_CALENDAR] != null) {
            ...
            if (i < events.length) {
              events[i] = this.resource;
              return localStorage[LOCAL_CALENDAR] = JSON.stringify(events);
            }
          }
        } else {
    ...
    Event.prototype.insert = function() {
      var events,
        _this = this;
      if ((this.calendarId == null) || this.calendarId === 'local') {
        events = localStorage[LOCAL_CALENDAR] != null ? JSON.parse(localStorage[LOCAL_CALENDAR]) : [];
        this.resource.id = new Date().getTime().toString();
        events.push(this.resource);
        return localStorage[LOCAL_CALENDAR] = JSON.stringify(events);
      } else {
    ...
    $('#modal-calendar').on('show', function(event) {
      ...
      return req.execute(function(resp) {
        ...
        if (resp.error != null) {
          ...
          return $calendarList.html('<option value="local">アプリ内カレンダー</option>' + ((function() {
            ...
            return _results;
          })()).join('') + '<option value="new">新規Goolgeカレンダー</option>');
        }
      });
    });
    $('#button-show').on('click', function() {
      var currentCalendar, e, event, events, i, id, name, options, req, time, timeMax, timeMin, _i, _len, _results;
      ...
      if ($('#form-calendar [name="start-date"]')[0].value !== '') {
        timeMin = $('#form-calendar [name="start-date"]')[0].value + 'T00:00:00Z';
      }
      if ($('#form-calendar [name="end-date"]')[0].value !== '') {
        timeMax = $('#form-calendar [name="end-date"]')[0].value + 'T00:00:00Z';
      }
      switch (id) {
        case 'local':
          events = localStorage[LOCAL_CALENDAR] != null ? JSON.parse(localStorage[LOCAL_CALENDAR]) : [];
          ...
          if (name = prompt('新しいカレンダーに名前をつけてください')) {
            ...
            return req.execute(function(resp) {
              ...
              if (resp.error != null) {
                ...
                return Event.changeCalendarId(currentCalendar.id);
              }
            });
          }
          break;
          ...
          return req.execute(function(resp) {
            ...
            if (resp.error != null) {
              ...
              for (i = _j = 0, _len1 = _ref1.length; _j < _len1; i = ++_j) {
                e = _ref1[i];
                _results1.push(event = new Event(id, e));
              }
              return _results1;
            }
          });
  ...
  initializeGoogleMaps = function(callback) {
    ...
    return map.addListener('click', function(event) {
      return new Event(typeof currentCalendar !== "undefined" && currentCalendar !== null ? currentCalendar.id : void 0, {
        extendedProperties: {
          "private": {
