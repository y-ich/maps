...
(function() {
  var CLIENT_ID, Event, MAP_STATUS, Place, SCOPES, TIME_ZONE_HOST, calendars, compareEventResources, currentCalendar, currentPlace, directionOrigin, directionsRenderer, geocoder, getLocalizedString, getTimeZone, handleAuthResult, initializeDOM, initializeGoogleMaps, localTime, localize, map, saveMapStatus, searchDirections, setLocalExpressionInto, spinner, timeDifference,
    _this = this,
  ...
  searchDirections = function(origin, destination, travelMode) {
  ...
  Place = (function(_super) {
    ...
    function Place(options, event, address) {
      ...
      google.maps.event.addListener(this, 'click', function() {
        if (directionOrigin != null) {
          searchDirections(directionOrigin, _this.getPosition(), google.maps.TravelMode.TRANSIT);
          return directionOrigin = null;
        } else {
    ...
    Place.prototype._setInfo = function() {
      var endDeferred, setTime, startDeferred,
        _this = this;
      ...
      if ((this.event.resource.start.date != null) && (this.event.resource.end.date != null)) {
        ...
        setTime = function(startOrEnd) {
          var dateTime;
          dateTime = _this.getDateTime(startOrEnd);
          Place.$modalInfo.find("input[name=\"" + startOrEnd + "-date\"]").val(dateTime.date);
          return Place.$modalInfo.find("input[name=\"" + startOrEnd + "-time\"]").val(dateTime.time);
        };
  ...
  initializeDOM = function() {
    ...
    $('#button-prev, #button-next').on('click', function() {
      var EventIndex, candidateIndex, sorted, _ref, _ref1;
      sorted = Event.events.sort(function(x, y) {
        return compareEventResources(x.resource, y.resource);
      });
      EventIndex = currentPlace != null ? sorted.indexOf(currentPlace.event) : 0;
    ...
    return $('#button-direction').on('click', function(event) {
      ...
      return alert('目的地のマーカをクリックしてください');
    });
  };
