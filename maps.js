...
(function() {
  var $addressField, $destinationField, $gps, $map, $message, $originField, $pinList, Bookmark, DEFAULT_ICON_SIZE, MSMARKER_SHADOW, MapFSM, MapState, MarkerWithCircle, MobileInfoWindow, PURPLE_DOT_IMAGE, RED_DOT_IMAGE, bookmarkContext, bookmarks, currentBookmark, currentLocationMarker, directionsRenderer, droppedBookmark, generateBookmarkList, generateHistoryList, geocoder, getArriveAtMessage, getDepartAtMessage, getLocalizedString, getMapType, getPanoramaHandler, getRouteIndexMessage, getTravelMode, history, infoWindow, initializeDOM, initializeGoogleMaps, isHold, localize, map, mapFSM, mapSum, maxHistory, meterToString, method, name, naviMarker, navigate, ordinal, pinRowHeight, saveMapStatus, saveOtherStatus, searchAddress, searchBookmark, searchDirections, secondToString, setInfoPage, setLocalExpressionInto, sum, tracer, updateField, updateMessage, _ref, _ref1, _ref2, _ref3, _ref4,
  ...
  MapState.TRACE_POSITION.update = function() {
    if (currentLocationMarker.getVisible()) {
  ...
  setInfoPage = function(bookmark, dropped) {
    ...
    $('#info-marker img:first-child').attr('src', (_ref5 = (_ref6 = bookmark.marker.getIcon()) != null ? _ref6.url : void 0) != null ? _ref5 : 'http://maps.google.co.jp/mapfiles/ms/icons/red-dot.png');
      ...
      if (/history/.test(name)) {
        ...
        switch (bookmarkContext) {
          case 'address':
            ...
            break;
          case 'origin':
            updateField($originField, name === 'currentLocationMarker' ? (latLng = bookmarkOrMarker.getPosition(), "" + (latLng.lat()) + ", " + (latLng.lng())) : bookmarkOrMarker.address);
            break;
          case 'destination':
            updateField($destinationField, name === 'currentLocationMarker' ? (latLng = bookmarkOrMarker.getPosition(), "" + (latLng.lat()) + ", " + (latLng.lng())) : bookmarkOrMarker.address);
        }
      }
      return $bookmarkPage.css('bottom', '-100%');
    });
