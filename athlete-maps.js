...
(function() {
  ...
  Array.prototype.find = function(predicate) {
    var ref;
    return (ref = this.filter(predicate)[0]) != null ? ref : null;
  };
  ...
  createKML = function(directionsRoute) {
    ...
    steps = [].concat.apply([], (function() {
      ...
      ref = directionsRoute.legs;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        e = ref[j];
        results.push(e.steps);
      }
      return results;
    })());
  ...
  elevationAlongSteps = function(steps, callback) {
    var MAGIC, aux, j, len, path, s, totalResult;
    MAGIC = 300;
    path = [];
    for (j = 0, len = steps.length; j < len; j++) {
      s = steps[j];
  ...
  drawElevation = function(elevationResults) {
    var aux, d, distances, elevations, i, j, maxSlope, maxSlopeIndex, minSlope, minSlopeIndex, ref, slope, slopes, steepBack, steepGo, threshold;
    ...
    for (i = j = 1, ref = elevationResults.length - 1; 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
  ...
  $('#elevation').on('submit', function(event) {
    var directions, directionsInHistory, e, index, j, len, ref, ref1, ref2, steps;
    directions = directionsRenderer.getDirections();
    index = (ref = directionsRenderer.getRouteIndex()) != null ? ref : 0;
    console.log(history, directions);
    directionsInHistory = history.find(function(e) {
      return e.directionsResult.routes[0] === directions.routes[0];
    });
    if (((ref1 = directionsInHistory.elevationResults) != null ? ref1[index] : void 0) != null) {
      drawElevation(directionsInHistory.elevationResults[index]);
    } else {
      steps = [];
      ref2 = directions.routes[index].legs;
  ...
  $('#kml').on('click', function() {
    ...
    xhr = $.ajax($this.attr('href').replace(/comgoogleearth/, 'http'), {
      ...
      data: createKML(directionsRenderer.getDirections().routes[(ref = directionsRenderer.getRouteIndex()) != null ? ref : 0])
    });
    if (xhr.status === 200) {
      return true;
    } else {
