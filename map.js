...
(function() {
  var $addressField, $destinationField, $gps, $map, $originField, $pinList, Bookmark, MSMARKER_SHADOW, MapFSM, MapState, MobileInfoWindow, PURPLE_DOT_IMAGE, RED_DOT_IMAGE, WatchPosition, bookmarkContext, bookmarks, currentBookmark, directionsRenderer, droppedBookmark, generateBookmarkList, generateHistoryList, geocoder, getLocalizedString, getMapType, getPanoramaHandler, getTravelMode, history, infoWindow, initializeDOM, initializeGoogleMaps, isHold, localize, makeInfoMessage, map, mapFSM, mapSum, maxHistory, meterToString, method, name, naviMarker, navigate, pinRowHeight, pulsatingMarker, saveMapStatus, saveOtherStatus, searchAddress, searchBookmark, searchDirections, secondToString, setInfoPage, sum, traceHandler, updateField, _ref, _ref1,
  ...
  getLocalizedString = function(key) {
    ...
    if (typeof localizedStrings !== "undefined" && localizedStrings !== null) {
      return (_ref2 = localizedStrings[key]) != null ? _ref2 : key;
    } else {
      return key;
    }
  };
  ...
  localize = function() {
    ...
    return $('#save-bookmark').text(getLocalizedString('Save'));
  };
  ...
  searchDirections = function(fromHistory) {
    ...
    return searchDirections.service.route({
      ...
      var $message, distance, duration, index, message, mode, summary;
      $message = $('#message');
      message = '';
      switch (status) {
  ...
  navigate = function(str) {
    var lengths, route, step, steps, _ref2;
    route = (_ref2 = directionsRenderer.getDirections()) != null ? _ref2.routes[directionsRenderer.getRouteIndex()] : void 0;
    if (route == null) {
      return;
    }
    switch (str) {
  ...
  setInfoPage = function(bookmark, dropped) {
    var position, title, _ref2, _ref3;
    console.log(bookmark.marker.getIcon());
    $('#info-marker img:first-child').attr('src', (_ref2 = (_ref3 = bookmark.marker.getIcon()) != null ? _ref3.url : void 0) != null ? _ref2 : 'http://maps.google.co.jp/mapfiles/ms/icons/red-dot.png');
  ...
  initializeGoogleMaps = function() {
    ...
    google.maps.event.addListener(map, 'click', function(event) {
      ...
      if ($infoWindow.length > 0) {
        ...
        if (((position.left <= (_ref2 = xy.x) && _ref2 <= position.left + $infoWindow.width())) && ((position.top <= (_ref3 = xy.y) && _ref3 <= position.top + $infoWindow.height()))) {
          return;
        }
      }
      infoWindow.close();
  ...
  initializeDOM = function() {
    ...
    if (localStorage['maps-other-status'] != null) {
      ...
      _ref3 = (_ref2 = otherStatus.bookmarks) != null ? _ref2 : [];
      ...
      history = (_ref4 = otherStatus.history) != null ? _ref4 : [];
    }
    localize();
    ...
    $addressField.on('keyup', function() {
      return $('#done').text($(this).val() === '' ? getLocalizedString('Done') : getLocalizedString('Cancel'));
    });
    ...
    $('#nav-bookmark button').on('click', function() {
      ...
      switch ($this.attr('id')) {
        case 'bookmark':
          $('#bookmark-message').text(getLocalizedString('Choose a bookmark to view on the map'));
          $('#bookmark-edit').text(getLocalizedString('Edit')).addClass('disabled');
          return generateBookmarkList();
        case 'history':
          $('#bookmark-message').text(getLocalizedString('Choose a recent search'));
          $('#bookmark-edit').text(getLocalizedString('Clear')).removeClass('disabled');
