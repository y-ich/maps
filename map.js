...
(function() {
  var $destination, $gps, $map, $origin, Bookmark, MapFSM, MapState, PURPLE_DOT_IMAGE, RED_DOT_IMAGE, WatchPosition, bookmarkContext, bookmarks, currentBookmark, directionsRenderer, droppedBookmark, geocodeHandler, geocoder, getMapType, getPanoramaHandler, getTravelMode, infoWindow, initializeDOM, initializeGoogleMaps, makeInfoMessage, map, mapFSM, mapSum, meterToString, method, name, naviMarker, navigate, pulsatingMarker, routeHistory, saveMapStatus, saveOtherStatus, searchDirections, searchHistory, secondToString, setInfoPage, traceHandler, _ref;
  ...
  initializeDOM = function() {
    var $edit, $mapType, $navi, $option, $route, $routeSearchFrame, $search, $traffic, $travelMode, $versatile, backToMap, e, otherStatus, pinRowHeight, trafficLayer, watchPosition, _i, _len, _ref1;
    $origin = $('#origin');
    ...
    if (localStorage['maps-other-status'] != null) {
      ...
      if ((otherStatus.origin != null) && otherStatus.origin !== '') {
        $origin.val(otherStatus.origin).siblings('.btn-bookmark').css('display', 'none');
      }
      if ((otherStatus.destination != null) && otherStatus.destination !== '') {
        $destination.val(otherStatus.destination).siblings('.btn-bookmark').css('display', 'none');
      }
      _ref1 = otherStatus.bookmarks;
    ...
    $('.btn-bookmark').on('click', function() {
      ...
      for (i = _j = 0, _len1 = bookmarks.length; _j < _len1; i = ++_j) {
        e = bookmarks[i];
