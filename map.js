...
(function() {
  var $gps, geocoder, getLocationHandler, image, latlng, map, marker, myMarker, myOptions, position, traceHandler, traceHeadingEnable, watchId;

  watchId = null;

  traceHeadingEnable = false;

  latlng = localStorage['position'] != null ? (position = JSON.parse(localStorage['position']), new google.maps.LatLng(position.lat, position.lng)) : new google.maps.LatLng(35.660389, 139.729225);
  ...
  window.onpagehide = function() {
    ...
    pos = map.getCenter();
    return localStorage['position'] = JSON.stringify({
      lat: pos.lat(),
      lng: pos.lng()
    });
  };
